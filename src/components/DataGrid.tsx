...
interface Props {
  ...
  onNavigateFK?: (fkSchema: string, fkTable: string, fkColumn: string, value: string) => void;
...
function renderCell(cell: CellValue): ReactNode {
  ...
  if (cell.type === "Geo") {
    ...
    const display = wkt
      ...
      : "geometry";
    return <span className="text-blue-500 text-xs font-mono truncate block" title={wkt ?? "geometry"}>{display}</span>;
  }
  if (cell.type === "Bool") {
    return (
...
function BoolCell({
  ...
  const current = hasPendingEdit ? pendingValue : (cell.type === "Bool" ? cell.value : null);
  ...
  return (
    <select
      ...
      onChange={(e) => {
        ...
        onCommit(v === "" ? null : v === "default" ? "DEFAULT" : v === "true" ? true : false);
      }}
      onMouseDown={(e) => e.stopPropagation()}
      onClick={(e) => e.stopPropagation()}
      className={`w-full bg-transparent border-0 outline-none text-sm cursor-pointer ${
        hasPendingEdit ? "text-amber-300" :
        current === true ? "text-green-600" :
        current === false ? "text-red-500" :
...
function FKCell({
  ...
  const value = cell.type !== "Null" ? String((cell as { type: string; value: unknown }).value ?? "") : null;
  ...
  return (
    <span className="flex items-center gap-1">
      ...
      {value !== null && (
        <button
          className="text-blue-500 hover:text-blue-400 text-xs shrink-0 leading-none"
...
function EditableCell({ cell, isEditing, isPendingEdit, onCommit, onCancel, onTabNext }: EditableCellProps) {
  const initialValue = getCellRawValue(cell);
  const [draft, setDraft] = useState(initialValue === null ? "" : String(initialValue));
  const inputRef = useRef<HTMLInputElement>(null);
  const originalRef = useRef<string>(initialValue === null ? "" : String(initialValue));
  ...
  function commitIfChanged(value: string) {
    ...
    const originalNormalized = originalRef.current === "" ? null : originalRef.current;
  ...
  if (isEditing) {
    return (
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") { e.preventDefault(); commitIfChanged(draft); }
          else if (e.key === "Escape") { e.preventDefault(); onCancel(); }
          else if (e.key === "Tab") { e.preventDefault(); commitIfChanged(draft); onTabNext(); }
        }}
  ...
  if (isPendingEdit) {
    return (
      <span className="truncate block text-amber-300" title={draft}>
        {draft === "" || draft === "null" ? <span className="italic text-xs text-amber-400/60">NULL</span> : draft}
      </span>
    );
  }

  return (
    <span className="truncate block w-full h-full">
      {renderCell(cell)}
    </span>
  );
}
...
export function DataGrid({
  ...
  const pkColName = tableSchema?.columns.find((c) => c.is_primary_key)?.name ?? null;
  const canEdit = pkColName !== null;

  const fkMap = new Map<string, { fkSchema: string; fkTable: string; fkColumn: string }>();
  if (tableSchema) {
    for (const col of tableSchema.columns) {
      if (col.is_foreign_key && col.fk_schema && col.fk_table && col.fk_column) {
        fkMap.set(col.name, { fkSchema: col.fk_schema, fkTable: col.fk_table, fkColumn: col.fk_column });
      }
    }
  }
  ...
  const deletedRowIndices = new Set<number>(deleteChange?.kind === "delete" ? deleteChange.rowIndices : []);
  ...
  const insertChanges = pendingChanges.filter((c): c is Extract<PendingChange, { kind: "insert" }> => c.kind === "insert");

  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    if (editingCell) return;
    if ((e.key === "Delete" || e.key === "Backspace") && selectedRowIndices.length > 0) {
      e.preventDefault();
  ...
  function getTabNextCell(rowIndex: number, colName: string): { rowIndex: number; colName: string } | null {
    if (!result) return null;
    const cols = result.columns.filter((c) => !c.is_geo && c.name !== pkColName);
    const idx = cols.findIndex((c) => c.name === colName);
    if (idx < cols.length - 1) return { rowIndex, colName: cols[idx + 1].name };
    if (rowIndex < result.rows.length - 1) return { rowIndex: rowIndex + 1, colName: cols[0]?.name ?? colName };
    return null;
  }
  ...
  const columns: ColumnDef<CellValue[], unknown>[] = (result?.columns ?? []).map(
    (col, i) => ({
      ...
      cell: ({ row }: { row: { original: CellValue[]; index: number } }) => {
        ...
        if (isDeleted) {
          return <span className="line-through text-muted-foreground/50 truncate block">{renderCell(row.original[i])}</span>;
        }
        ...
        const pendingVal = hasPendingEdit ? updatedCells.get(cellKey)! : getCellRawValue(row.original[i]);
        const isBool = row.original[i]?.type === "Bool" ||
        ...
        return (
          <EditableCell
            cell={hasPendingEdit
              ? (pendingVal === null ? { type: "Null" } : { type: "Text", value: String(pendingVal) })
              : row.original[i]}
            isEditing={isEditing}
    ...
    <div className="flex flex-col flex-1 overflow-hidden border rounded-md">
      ...
      {hasPendingChanges && (
        <div className="flex items-center gap-3 px-3 py-1 border-b bg-amber-500/10 text-xs text-amber-400 shrink-0">
          ...
          <span className="text-muted-foreground">· ⌘S to commit · ⌘R to discard</span>
        </div>
      )}
        ...
        >
          <thead className="bg-muted/40 sticky top-0 z-10">
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id}>
                <th className="w-8 px-2 py-1.5 text-left text-xs text-muted-foreground border-b border-r font-normal select-none" style={{ width: 32, minWidth: 32 }}>
                  #
                </th>
                {hg.headers.map((h) => {
                  const colMeta = result.columns[parseInt(h.column.id.replace("col-", ""))];
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className="font-medium truncate">{h.column.columnDef.header as string}</span>
                        {colMeta && (
                        ...
                        {colMeta?.is_geo && (
                          <Badge variant="secondary" className="text-xs px-1 py-0 shrink-0">geo</Badge>
                        )}
                        {sorted === "asc" && <span className="text-xs shrink-0">↑</span>}
                        {sorted === "desc" && <span className="text-xs shrink-0">↓</span>}
                        {!sorted && <span className="text-xs text-muted-foreground/30 shrink-0">↕</span>}
                      </div>
          ...
          <tbody>
            {table.getRowModel().rows.map((row, displayIdx) => {
              ...
              let rowClass = "border-b last:border-b-0 cursor-pointer transition-colors ";
              if (isDeleted) {
              ...
              return (
                ...
                >
                  <td
                    ...
                    onMouseDown={(e) => { e.stopPropagation(); onRowSelect?.(dataIdx, e); }}
                  >
                    {displayIdx + 1}
                  </td>
                  {row.getVisibleCells().map((cell) => {
                    ...
                    const hasPendingEdit = updatedCells.has(cellKey);
                    const isEditing = editingCell?.rowIndex === dataIdx && editingCell?.colName === colName;
                    const isPK2 = colName === pkColName;
                    const isGeo2 = result.columns[colIdx]?.is_geo ?? false;
                    const canEditCell = canEdit && !isPK2 && !isGeo2 && !!onCellEdit;
                    const isBoolCol = cell.getContext().row.original[colIdx]?.type === "Bool" ||
                      (hasPendingEdit && (updatedCells.get(cellKey) === true || updatedCells.get(cellKey) === false));

                    let tdClass = "px-3 py-1 border-r last:border-r-0 overflow-hidden ";
                    if (isEditing) tdClass += "ring-1 ring-inset ring-blue-500 bg-blue-500/5 ";
                    else if (hasPendingEdit) tdClass += "bg-amber-500/15 border-amber-500/40 ";
                    ...
                    return (
                      <td
                        ...
                        onMouseDown={canEditCell && !isDeleted && !isBoolCol ? (e) => {
                          if (e.detail === 2) {
                            e.preventDefault();
                            setEditingCell({ rowIndex: dataIdx, colName });
            ...
            {insertChanges.map((ins, insIdx) => {
              ...
              const isMultiSelected = selectedRowIndices.includes(virtualRowIdx);
              return (
                <tr
                  key={ins.tempId}
                ...
                >
                  <td className="px-2 py-1 text-xs text-green-500/70 select-none border-r" style={{ width: 32, minWidth: 32 }}>
                    +
                  </td>
                  {(result?.columns ?? []).map((col, colIdx) => {
                    const val = ins.values[col.name];
                    const isEditing = editingCell?.rowIndex === virtualRowIdx && editingCell?.colName === col.name;
                    const isPK = col.name === pkColName;
                    const isGeo = col.is_geo;
                    let tdClass = "px-3 py-1 border-r last:border-r-0 overflow-hidden ";
                    if (isEditing) tdClass += "ring-1 ring-inset ring-green-500 bg-green-500/5 ";

                    return (
                      <td key={colIdx} className={tdClass} style={{ width: 160, maxWidth: 160 }}>
                        {isPK || isGeo ? (
                          <span className="text-muted-foreground/30 italic text-xs">auto</span>
                        ) : (
                          <EditableCell
                            cell={val !== undefined && val !== null ? { type: "Text", value: String(val) } : { type: "Null" }}
                            isEditing={isEditing}
                            ...
                            onCancel={() => setEditingCell(null)}
                            onTabNext={() => {
                              const editableCols = result.columns.filter((c) => !c.is_geo && c.name !== pkColName);
                              const idx = editableCols.findIndex((c) => c.name === col.name);
                              if (idx < editableCols.length - 1) {
                                setEditingCell({ rowIndex: virtualRowIdx, colName: editableCols[idx + 1].name });
                              } else {
                                setEditingCell(null);
                              }
                            }}
